...
- **`dent8 snapshot [--include-diagnostics] [--include-context]`** — emits one
  debugger/control-plane read/audit payload over the same durable store: live runtime status,
  fact-stream browsing (index only), `verify`, `conflicts`, and summary counts that promote
  **witness status**, **unwitnessed tail**, and **attention** strings so operators/agents do
  not miss nested WARNs. Optional `--include-context` nests a full belief pack (values +
  authority + source). In text mode it is a compact operator status; in `--output json` it is
  the stable polling shape for local dashboards and daemon/MCP clients. It exits nonzero for
  integrity issues, degraded runtime inspection, or witness warn/fail; a merely contested
  belief base stays a successful read with `status: "contested"`.
...
- **`dent8 mcp serve`** — a stdio JSON-RPC 2.0 **MCP server** exposing read/audit tools
  ...
  `native_scan` / `native_reconcile`) and the
  ...
  **`context`** is the agent inject pack (CLI `dent8 context` parity: values + authority +
  source; optional filters, `detail=summary`, `record_retrieval`). `list_facts` remains the
  lightweight stream index. `runtime_status` reports the live server binary, cwd, selected
  store URL/path, event count, authority registry, signed identity, and witness configuration,
  while `snapshot` combines that runtime view with the fact index, `verify`, `conflicts`, and
  attention/witness summary fields (`include_context` nests a belief pack). **`explain`**
  accepts a single subject+predicate or a `queries[]` batch, with optional `detail=summary`.
  The initialize response instructs agents to call **`context`** (or `snapshot` for health)
  before relying on project facts, weight High human/CI above Low agent, use
  `native_reconcile` when export/rules may drift, and treat rejected writes as safety signals.
  Claude-eager tools are `runtime_status` / `context` / `assert` / `explain`.
...
- **`dent8 mcp proxy [--socket <path>]`** — a stdio MCP bridge to a running local daemon. It
  ...
  between stdin/stdout and the authenticated daemon connection. JSON-RPC notifications are
  forwarded without waiting for a reply, so normal MCP clients do not deadlock on
  `notifications/initialized`. This lets stdio-only clients use one long-lived daemon and one
...
- **`dent8 witness keygen | sign | verify | verify-published | head | publish | serve |
  ...
  current log's matching **prefix** and that the counts never decrease — catching a history
  **rewrite** (a re-hashed-forward edit an internal `verify_chain` cannot, threat-model T6) as
  `TAMPER` and a truncation/reorder as `ROLLBACK`. **`serve [interval] [max-heads]`** is the
  **cadence signer** — it signs the head whenever the log grows, the loop a separate operator
  runs; **`head`** prints the latest signed head as JSON; and
  **`publish <heads.jsonl>`** idempotently appends that latest local head to an external JSONL
  sequence, refusing to publish behind a sequence that is already ahead.
...
- **Reviewed legitimate-traffic traces** (`dent8 eval --trace <FILE>`): the strict
  ...
  The workflow and synthetic example are in [`evals/traces/`](../evals/traces/). Five reviewed,
  redacted maintainer-dogfood traces ship (Claude Code, Cursor, Grok Build, multi-op CLI,
  multi-agent team): **13** captured legitimate operations, 0 false positives. This is
  integration evidence, not independent external-user evidence; the designed benign corpus
  remains the broader 0/22 tally. Gate with [`scripts/integrity-check.sh`](../scripts/integrity-check.sh);
  monorepo dogfood write-check via `scripts/dogfood-doctor.sh --write-check`; checklist in
...
- **Postgres remaining gaps — not the adapter itself.** The v0
  ...
  `assert → supersede → explain → verify` end-to-end). The stock binary still defaults to the
  file dev store but includes SQLite for no-server shared stores. The async side is a backend-agnostic **`AsyncEventStore`** trait
  ...
  `sqlite://` URL, included in stock CLI builds) is **runnable + tested** — assert/supersede/explain/verify
