...
`facts list`, `verify`, `conflicts`, `eval`, `authority`, `identity status`, and `doctor`
support `--output json` for scripts/agents; other commands fail closed when JSON is requested
until their structured contract is designed.

**Remaining.** JSON contracts for setup/configuration commands beyond `authority` and
`identity status`, `replay_runs` persistence, `--as-of` / `--valid-at`, `valid_to` intervals,
