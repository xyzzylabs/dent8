...
`identity status`, `doctor`, and `mcp install` support `--output json` for scripts/agents; other commands fail closed when
...
`identity status`, `init`, `agent add`, and `mcp install`, `replay_runs` persistence, `--as-of` /
