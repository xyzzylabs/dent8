...
5. **Identity productization.** ✅ All three platforms: `keychain:<account>` is accepted
   wherever a key path is (`DENT8_IDENTITY_KEY`, keygen `--out`, `--issuer-key`,
   `--public-key`) — the key lives in the OS keychain (macOS Keychain; any Linux Secret
...
**first-class in-process framework adapters — done for the major frameworks**: LangChain
(Python, `dent8.langchain`), LlamaIndex (Python, `dent8.llamaindex`), the Vercel AI SDK
(`dent8/ai`), and LangChain.js (`dent8/langchain`), all native tools over the SDKs, plus a
framework-agnostic `dent8/tools` (TS) / `dent8ToolSpecs` for anything else (see
[examples/langchain](../examples/langchain/), [examples/llamaindex](../examples/llamaindex/),
