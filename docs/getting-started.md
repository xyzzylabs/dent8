...
Every command below was run against the real `dent8` v0.7.1 binary; the output blocks are
...
**Release binaries (recommended, v0.7.1+).** The [releases page][releases] ships prebuilt
archives for five targets, each with a `.sha256` sidecar (built with `postgres,sqlite`):

- `dent8-v0.7.1-aarch64-apple-darwin.tar.gz`
- `dent8-v0.7.1-x86_64-apple-darwin.tar.gz`
- `dent8-v0.7.1-aarch64-unknown-linux-gnu.tar.gz`
- `dent8-v0.7.1-x86_64-unknown-linux-gnu.tar.gz`
- `dent8-v0.7.1-x86_64-pc-windows-msvc.zip`

Download, verify, and install one target (Linux x86_64 shown):

```sh
BASE=https://github.com/xyzzylabs/dent8/releases/download/v0.7.1
curl -LO "$BASE/dent8-v0.7.1-x86_64-unknown-linux-gnu.tar.gz"
curl -LO "$BASE/dent8-v0.7.1-x86_64-unknown-linux-gnu.tar.gz.sha256"
sha256sum -c dent8-v0.7.1-x86_64-unknown-linux-gnu.tar.gz.sha256
tar xzf dent8-v0.7.1-x86_64-unknown-linux-gnu.tar.gz
...
> The `cargo install dent8-cli --version 0.7.1 --locked` path was verified in a clean install
> root as part of cutting v0.7.1 (the built binary reports `dent8 0.7.1`); a from-source build
