...
rows at once**: Claude Code through the tracked hooks in
[`.claude/settings.json`](../.claude/settings.json), and Codex, Cursor, and Grok through the
tracked [`AGENTS.md`](../AGENTS.md) contract (the fact base is authoritative for all of them)
plus machine-local MCP configs with per-agent signed identities — generated by
