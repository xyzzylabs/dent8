...
| `DENT8_IDENTITY_KEY` | every write | *(unset)* | Source private signing key: a file path (owner-only `0600` on Unix) or `keychain:<account>` for an OS-keychain-backed key (macOS Keychain, or Linux Secret Service via `secret-tool`; service `dent8`). |
