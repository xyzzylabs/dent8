...
| `DENT8_GRANT` | every write | *(unset)* | Signed source grant JSON binding the configured source id to a source public key and maximum authority. CLI writes use this grant to default missing `--source` and `--authority` before the normal authority/identity checks run. |
