...
| `DENT8_AUTHORITY` | `dent8 init`, `dent8 authority` + every write | discovered `.dent8/authority.json` (in-repo), else `./dent8-authority.json` | Path to the source→authority **ceiling** registry. When unset, it resolves against the discovered `.dent8/` store — a discovered `.dent8/env` `DENT8_AUTHORITY` if set, else the store's `authority.json` (the same file `dent8 init` seeds) — so `init` and the `authority` subcommands share one registry per store. Ceiling enforcement is **opt-in**: it activates only once this file exists (created by `dent8 init` or `dent8 authority add`); then it is deny-by-default. This gates the authority **ceiling** only — separately and unconditionally, any write above the agent tier (medium/high/canonical) requires a valid signed identity whether or not this registry exists (see `DENT8_TRUST` / `DENT8_REQUIRE_IDENTITY`). |
...
| `DENT8_TRUST` | signed identity | `./dent8-trust.json` | Path to trusted issuer public keys, used to verify signed writes. **Any write above the agent tier (medium/high/canonical) always requires a valid signed identity** — it is rejected outright with no trust registry/grant/key configured, regardless of the authority registry or the flags below — so this is not optional for high-authority use. When the file exists, signed identity is also verified for agent-tier writes. |
...
| `DENT8_REQUIRE_IDENTITY` | every write | *(unset / false)* | Fail-closed identity guard. Above-agent writes (medium/high/canonical) require a valid signed identity even when this is unset; setting it **additionally** fail-closes agent-tier `low` writes, rejecting them when a trust registry, grant, or source key is missing. |
...
| `DENT8_IDENTITY_KEY` | every write | *(unset)* | Source private signing key: a file path (owner-only `0600` on Unix) or `keychain:<account>` for an OS-keychain-backed key (macOS Keychain, Linux Secret Service via `secret-tool`, or Windows Credential Manager; service `dent8`). |
