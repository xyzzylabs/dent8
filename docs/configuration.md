...
| `DENT8_IDENTITY_KEY` | every write | *(unset)* | Source private signing key: a file path (owner-only `0600` on Unix) or `keychain:<account>` for an OS-keychain-backed key (macOS; service `dent8`). |
