...
|---|---|---|---|
| `DENT8_LOG` | CLI / MCP (file backend) | discovered `.dent8/` upward, else `./dent8-log.jsonl` | Path to the JSON-lines dev-store log. When unset, the CLI walks up from the current directory to the first ancestor containing a `.dent8/` store and uses the log inside it (matching what `.dent8/env` records), so a sub-directory command hits the project store instead of a parallel cwd log. Only with no `.dent8/` upward does it fall back to `./dent8-log.jsonl`. |
...
| `DENT8_AUTHORITY` | `dent8 init`, `dent8 authority` + every write | discovered `.dent8/authority.json` upward, else `./dent8-authority.json` | Path to the source→authority **ceiling** registry. When unset, it resolves to the `authority.json` inside the discovered `.dent8/` store (the same file `dent8 init` seeds), so `init` and the `authority` subcommands share one registry per store. Enforcement is **opt-in**: it activates only once this file exists (created by `dent8 init` or `dent8 authority add`); then it is deny-by-default. |
