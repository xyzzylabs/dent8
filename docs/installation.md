...
crate name is the library facade, for `cargo add dent8`). The current release is `0.7.3`.
...
```sh
cargo install dent8-cli --version 0.7.3 --locked
```
...
For Windows, download `dent8-v0.7.3-x86_64-pc-windows-msvc.zip` and its `.sha256`, verify
...
provenance matters. Remote HTTP/streamable MCP is future work, not part of the `0.7.3`
release.
