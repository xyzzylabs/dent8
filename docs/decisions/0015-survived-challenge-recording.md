...
(`InsufficientAuthority`), a supersession whose *backing fact* is actually weaker than its
stated authority (`LaunderedAuthority`), a retraction/expiration below the incumbent, a
contradiction against a canonical fact (`CanonicalContradiction`). Today a rejected
...
- **The strongest entrenchment signal.** A fact that was *attacked and stood* is better
  evidence than a fact nobody ever tested. `FactState.corroborating_sources` deliberately
...
Separately, [`SubjectProjection::unearned_supersessions`] audits *admitted* supersessions
after the fact — flagging `AuthorityDowngrade` and `WeakerCorroboration` — but nothing
enforces the corroboration half at write time: an equal-authority, single-source fact can
...
FactEventKind::ChallengeRejected {
    ...
    by: Option<FactId>,             // the challenging fact, when the challenge named one
...
- The event's `authority` is the challenger's **effective** authority — the stated level,
  except for a laundered supersession where it is the backing fact's *actual* (lower)
...
- The fold treats it as non-terminal bookkeeping on a live incumbent: it accumulates
  `FactState.survived_challenges: BTreeMap<SourceId, AuthorityLevel>` (challenger source →
...
- Recorded only for challenges that were **real contests lost on strength**: the four
  ...
  assertions, terminal-state mutations, and supersessions naming a nonexistent fact
...
gate* at the op boundary: an equal-authority supersession whose backing fact has strictly
...
challenge like any other. The `AuthorityDowngrade` half needs no gate: the subject-aware
...
- `FactState.survived_challenges` is `#[serde(default)]`, so previously materialized
...
- Survived challenges are consulted by arbitration as of
  ...
  gate (and the subject-level audit) weigh **earned entrenchment** = corroboration + survived
  challenges, so a fact that survived an equal-authority challenge resists the next fresh
