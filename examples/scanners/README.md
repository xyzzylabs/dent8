...
([docs/content-check.md](../../docs/content-check.md)): set `DENT8_CONTENT_CHECK_ARGV` to a
JSON argv array (or legacy `DENT8_CONTENT_CHECK` for simple whitespace-split commands), and
every candidate fact is piped to it as JSON before it can be arbitrated, attested, or persisted.
dent8 deliberately ships **no content classifier of its own** —
...
```sh
export DENT8_CONTENT_CHECK_ARGV="[\"$PWD/examples/scanners/demo-content-check.sh\"]"
...
config reference (`DENT8_CONTENT_CHECK_ARGV`, `DENT8_CONTENT_CHECK_TIMEOUT_MS`,
`DENT8_CONTENT_CHECK_FAIL_OPEN`) and the fail-closed rationale.
