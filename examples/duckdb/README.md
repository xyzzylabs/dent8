...
| `event_id`, `fact_id` | the event and the fact it acts on |
| `kind` | `fact.asserted`, `fact.superseded`, `fact.retracted`, … |
...
| `derived_from` | a **list** of source fact ids — the `DerivedFrom` dependency edges ([ADR 0010](../../docs/decisions/0010-evidence-edges-and-retraction-taint.md)); `UNNEST` it, don't string-split (a fact id may contain any character) |
...
# event timeline for one subject
