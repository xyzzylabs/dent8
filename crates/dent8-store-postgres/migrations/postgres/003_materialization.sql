...
-- The current folded `FactState` per fact. Upserted on every accepted append.
CREATE TABLE IF NOT EXISTS dent8_claim_projection (
    ...
    superseded_by TEXT,
    -- Facts that contradict this one (the accumulated `contradicted_by`).
    ...
    -- The exact folded `FactState`, for lossless reconstruction without re-folding. The
...
-- The fact->fact relationship graph. Each row is recorded from one originating event:
-- the event lives on `from_claim_id` and names `to_claim_id` (its `by`). Direction reads
-- "`from` records a `<edge_type>` link to `to`" — e.g. a `fact.superseded {by: B}` on A
-- yields (from=A, to=B, supersedes). `recorded_at` is the originating event's stamp (never
-- DB-generated), so replay rebuilds identical edges. (`uses_as_evidence` — fact->evidence,
