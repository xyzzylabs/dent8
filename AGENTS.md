...
- When dogfood state is present, consult dent8 for durable project facts before relying on
  remembered setup or preferences. Prefer MCP tools (`context`, `snapshot`, `explain`,
  `verify`) when available — `context` for belief packs (values + authority + source),
  `list_facts` only as a stream index; weight High human/CI above Low agent. Otherwise use
  the local CLI after loading `.dent8/env` and the agent-specific identity env (for example
  `.dent8/identity-codex.env`, `.dent8/identity-claude-code.env`, or
  `.dent8/identity-grok-build.env`).
...
<!-- dent8 export: 18 believed fact(s); do not edit inside this block — run `dent8 export --target <file>` to refresh -->

### eval:corpus

- `dent8://eval/corpus/captured.tally` = "5 reviewed redacted maintainer-dogfood traces (Claude Code, Cursor, Grok Build, multi-op CLI, multi-agent team) / 13 legitimate operations / 0 false positives; integration evidence, not independent external-user evidence — gate with scripts/integrity-check.sh; dogfood write-check via scripts/dogfood-doctor.sh --write-check"  <!-- dent8 receipt fact=fact:eval:corpus:captured.tally:12 event_hash=0ea87d7fa7e7… authority=high source=source:human -->
- `dent8://eval/corpus/core.tally` = "47 adversarial cases / 10 attack classes: 16 blocked by arbitration, 4 detect-only, 27 out-of-model (by design). Frozen: per_class_block_rates_match_the_frozen_honest_tally"  <!-- dent8 receipt fact=fact:eval:corpus:core.tally:10 event_hash=a8b2bc892c20… authority=high source=source:human -->
- `dent8://eval/corpus/hook.tally` = "content-check lane: 16 arbitration + 9 hook = 25 blocked, 9 detect-only, 13 admitted unflagged. Frozen: per_class_rates_with_the_demo_scanner_match_the_frozen_honest_tally"  <!-- dent8 receipt fact=fact:eval:corpus:hook.tally:11 event_hash=887702ecea0a… authority=high source=source:human -->

### hook:content-check

- `dent8://hook/content-check/contract` = "protocol dent8.content-check/1: DENT8_CONTENT_CHECK spawns a scanner; stdin=JSON payload, stdout=JSON verdict {allow|reject|taint}; exit 0 for any verdict (non-zero = scanner failed), fail-closed by default; runs in the op_* write layer after authority, before arbitration/persist"  <!-- dent8 receipt fact=fact:hook:content-check:contract:13 event_hash=03513e5149ad… authority=high source=source:human -->

### policy:authority

- `dent8://policy/authority/default.profile` = "human > CI > agent: source:human max=high, source:ci max=medium, source:agent max=low; seeded by `dent8 authority defaults`; deny-by-default for unlisted sources"  <!-- dent8 receipt fact=fact:policy:authority:default.profile:9 event_hash=f9effe8f373b… authority=high source=source:human -->

### repo:dent8

- `dent8://repo/dent8/cli.binary` = "CLI binary is `dent8` (crates/dent8-cli); tagline: a memory firewall for coding agents"  <!-- dent8 receipt fact=fact:repo:dent8:cli.binary:0 event_hash=434da937dde5… authority=high source=source:human -->
- `dent8://repo/dent8/commit.attribution` = "no Co-Authored-By trailers and no AI attribution in commits or PRs"  <!-- dent8 receipt fact=fact:repo:dent8:commit.attribution:7 event_hash=3e53f8ad21e5… authority=high source=source:human -->
- `dent8://repo/dent8/commit.author` = "single maintainer (chicoxyzzy / Sergey Rubanov) <chi187@gmail.com>"  <!-- dent8 receipt fact=fact:repo:dent8:commit.author:5 event_hash=3170c2384280… authority=high source=source:human -->
- `dent8://repo/dent8/commit.style` = "Conventional Commits: feat/fix/docs/test/ci, optional scope, trailing (#PR)"  <!-- dent8 receipt fact=fact:repo:dent8:commit.style:6 event_hash=80b6c44e9423… authority=high source=source:human -->
- `dent8://repo/dent8/gate.clippy` = "cargo clippy --workspace --all-targets -- -D warnings (warnings are errors)"  <!-- dent8 receipt fact=fact:repo:dent8:gate.clippy:4 event_hash=f86fedb4dcb2… authority=high source=source:human -->
- `dent8://repo/dent8/gate.fmt` = "cargo fmt --all -- --check"  <!-- dent8 receipt fact=fact:repo:dent8:gate.fmt:3 event_hash=97f1df0057a2… authority=high source=source:human -->
- `dent8://repo/dent8/gate.test` = "cargo test --workspace (CI job: fmt + clippy + test in .github/workflows/ci.yml)"  <!-- dent8 receipt fact=fact:repo:dent8:gate.test:2 event_hash=d7007f277d4a… authority=high source=source:human -->
- `dent8://repo/dent8/msrv` = "Rust 1.94 — Cargo.toml [workspace.package] rust-version = \"1.94\"; edition 2024; no rust-toolchain.toml (CI builds on stable)"  <!-- dent8 receipt fact=fact:repo:dent8:msrv:1 event_hash=4d979e9fe207… authority=high source=source:human -->
- `dent8://repo/dent8/product.next_arc` = "Ship Tauri shell over dent8 ui (ADR 0020 step 4), then signed write actions (step 5); keep the desktop app a control plane over CLI/MCP/daemon — never a separate write path"  <!-- dent8 receipt fact=fact:repo:dent8:product.next_arc:16 event_hash=d05fbae105e1… authority=high source=source:human -->
- `dent8://repo/dent8/release_status` = "workspace package version 0.8.0 on main (crates.io/GitHub release tags may lag; check Cargo.toml [workspace.package] version and git tags)"  <!-- dent8 receipt fact=fact:repo:dent8:release_status:17 event_hash=dc3d9827d808… authority=high source=source:human -->
- `dent8://repo/dent8/store.default` = "default store is a file dev log at .dent8/memory.jsonl; the .dent8/ store is gitignored (machine-local) — rebuild with scripts/dogfood-seed.sh"  <!-- dent8 receipt fact=fact:repo:dent8:store.default:8 event_hash=6d588290c459… authority=high source=source:human -->

### roadmap:dent8

- `dent8://roadmap/dent8/active.directions` = "Tauri desktop/control-plane work, operated witness hosting, and training/eval substrate work are active, not frozen; docs/STATUS.md remains authoritative for what is implemented, packaged, hosted, or design-only"  <!-- dent8 receipt fact=fact:roadmap:dent8:active.directions:15 event_hash=226b672fa98a… authority=high source=source:human -->
- `dent8://roadmap/dent8/current.focus` = "Post-v0.8 focus: agent-read ergonomics (MCP context pack, snapshot attention, batch explain), independent legitimate-traffic evidence, and product work on the Tauri control plane, hosted witness, and training/eval substrate"  <!-- dent8 receipt fact=fact:roadmap:dent8:current.focus:14 event_hash=1e8eae040636… authority=high source=source:human -->
