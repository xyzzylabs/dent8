...
`identity <subcommand>`, `doctor`, and `mcp install`; unsupported commands fail closed instead of silently returning
prose.
...
- `dent8 --output json <write-command>|explain|replay|facts list|verify|conflicts|eval|init|agent add|authority|identity <subcommand>|doctor|mcp install`:
