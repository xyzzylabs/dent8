...
`replay`, `facts list`, `verify`, `conflicts`, `eval`, `authority`, and `doctor`; unsupported
commands fail closed instead of silently returning prose.
...
- `dent8 --output json <write-command>|explain|replay|facts list|verify|conflicts|eval|authority|doctor`:
