...
`replay`, `facts list`, `verify`, `conflicts`, `eval`, `authority`, `identity status`, and
`doctor`; unsupported commands fail closed instead of silently returning prose.
...
- `dent8 --output json <write-command>|explain|replay|facts list|verify|conflicts|eval|authority|identity status|doctor`:
